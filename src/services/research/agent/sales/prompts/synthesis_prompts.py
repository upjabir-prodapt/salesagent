...
**Step B — `colt_product_search`:** Identify maximum target challenges. For each target challenge you plan to map, search the **Colt product catalog** for concrete product names/snippets that address that need.
...
1. {PLANNING_TAG} — Map each report section below to the listed output keys; note any gaps.
2. {REASONING_TAG} — Write the **complete markdown draft** following the exact structure below.
3. {ACTION_TAG} — Call `{REPORT_VERIFICATION_AGENT_NAME}` with `request` set to the **full draft markdown** (entire report text).
4. If validation returns **FAILED**: {REPLANNING_TAG} — fix every listed violation, revise the draft, call `{REPORT_VERIFICATION_AGENT_NAME}` again.
5. Only after validation returns **PASSED**: emit {FINAL_ANSWER_TAG} with the verified report only — no new facts, no extra tool calls.

| Phase | Your job |
|-------|----------|
| {PLANNING_TAG} | Section-to-output-key map; flag missing data as `Data not available from research.` |
| {REASONING_TAG} | Full markdown draft per structure below |
| `{REPORT_VERIFICATION_AGENT_NAME}` | Submit entire draft; read `violations` and `message` on FAILED |
| {REPLANNING_TAG} | Targeted fixes only — do not re-run research agents |
| {FINAL_ANSWER_TAG} | Verified markdown report only |
...
- Extract data from each **injected agent output** above for the matching report sections.

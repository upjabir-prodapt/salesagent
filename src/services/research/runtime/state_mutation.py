...
from .retry.errors import RETRY_SCOPE_RUNNER_COLD, classify_error, retry_scope_for_error_class
...
def requires_cold_retry(exc: Exception) -> bool:
    ...
    if isinstance(exc, AgentOutputError):
        ...
        if error_class == "REPORT_VALIDATION_FAILED":
            logger.info(
                f"[Retry] Fail-fast decision agent={exc.agent_name} "
        ...
        if _MISSING_OUTPUT_PHRASE in msg:
            logger.info(
                f"[Retry] Cold-retry decision agent={exc.agent_name} "
                f"reason=missing_output_phrase"
            )
            return True
        if any(marker in msg for marker in _MALFORMED_MARKERS):
            logger.info(
                f"[Retry] Cold-retry decision agent={exc.agent_name} "
                f"reason=malformed_marker"
            )
            return True
        logger.info(

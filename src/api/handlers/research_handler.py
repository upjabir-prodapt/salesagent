...
from src.shared.utils.guardrails import InputGuardrail
...
class ResearchHandler:
    """Maps research HTTP operations to ResearchJobService and Cloud Tasks."""

    def __init__(
        self,
        service: ResearchJobService,
        cloud_tasks_service: CloudTasksService | None = None,
    ) -> None:
        self._service = service
        self._cloud_tasks = cloud_tasks_service
        ...
        with contextualize(
            ...
            with tracer.start_as_current_span("research.request.accepted") as span:
                ...
                if settings.API_USE_BACKGROUND_PIPELINE:
                    ...
                    tasks_service = self._cloud_tasks or CloudTasksService()
                    try:
                        await asyncio.to_thread(
                            tasks_service.enqueue_research,
                            job_id,
                            request.company_name,
                            metadata,
                        ...
                        span.set_attribute("research.tasks_enqueued", True)
                    except Exception as e:
                        logger.error(
                            f"Failed to enqueue Cloud Task for job {job_id}: {e}"
                        )
                        self._service.mark_job_failed(job_id, f"Enqueue error: {e}")
                        span.set_attribute("research.tasks_enqueued", False)
                        raise ServiceError(
                            f"Failed to enqueue research task: {e}"
                        ) from e

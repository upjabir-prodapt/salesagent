    ...
    async def _check_hallucinations_legacy(
        ...
        try:
            ...
            prompt = (
                ...
                "within Section 11 (using the `[Source: <agent> — \"<fact>\"]` format). "
                "A claim is UNSUPPORTED ONLY if:\n"
                "  - It references a specific event (fine, breach, acquisition, initiative) "
                "that has no corresponding signal in Section 12 AND no inline citation in Section 11.\n"
                "  - It uses a specific figure (revenue, headcount, latency target) not present "
                "in Section 12 signals AND not explicitly cited in Section 11.\n"
                "  - It asserts urgency (e.g. 'regulatory pressure', 'AI rollout underway') "
                "with no signal entry OR inline citation providing evidence.\n"
                "  - The Section 12 signal OR the inline Section 11 citation it refers to does "
                "not actually contain the stated fact.\n\n"
                "DO NOT flag as unsupported:\n"
                "  - **Claims with explicit inline citations**: If a bullet point ends with "
                "`[Source: <agent_output_key> — \"<exact data point or quote>\"]`, and that cited "
                "fact supports the claim, it is VALID even if it doesn't appear in Section 12.\n"
                "  - **Strategic Suggestions or Recommendations**: Any advice, suggested strategy, or proposed solution for the target company is an analytical output and should NOT be flagged.\n"
                "  - **Mentions of Incumbent Providers**: References to existing vendors (e.g. BT, Orange, Vodafone) are standard sales intelligence and should NOT be flagged.\n"
                "  - **Sales Hypotheses / Pain Points**: Claims about 'aging infrastructure', 'legacy bottlenecks', or 'performance gaps' are strategic displacement angles and do not require specific citations.\n"
                "  - Competitive displacement arguments or sales inferences "

...
_JAILBREAK_PATTERNS: list[tuple[str, str]] = [
    ...
    ("act_as", r"(you\s+are\s+now|act\s+as|pretend\s+(to\s+be|you\s+are))"),
    ...
    async def _check_hallucinations_with_cache(
        ...
        try:
            ...
            prompt = (
                ...
                "  - It states a specific number (revenue, headcount, growth rate, market share, "
                "fine amount, contract value) that does not appear in the evidence.\n"
                "  - It references a named event (acquisition, breach, regulatory action, product "
                "launch, partnership) that has no corresponding entry in the evidence.\n"
                "  - It asserts urgency or a trend ('undergoing transformation', 'facing pressure') "
                "with no evidence snippet supporting it.\n"
                "  - It names a specific executive, title, or bio detail not found in the evidence.\n"
                "DO NOT flag:\n"
    ...
    async def _check_hallucinations_legacy(
        ...
        try:
            ...
            prompt = (
                ...
                "Section 12 (Signals). Section 11 is written by a synthesis agent that may "
                "hallucinate urgency, regulatory events, or AI initiatives that were never found "
                ...
                "whether the claims are supported by Section 12. A claim is UNSUPPORTED if:\n"
                "  - It references a specific event (fine, breach, acquisition, initiative) "
                "that has no corresponding signal in Section 12.\n"
                "  - It uses a specific figure (revenue, headcount, latency target) not present "
                "in Section 12 signals.\n"
                "  - It asserts urgency (e.g. 'regulatory pressure', 'AI rollout underway') "
                "with no signal entry providing evidence.\n"
                "  - The Section 12 signal it cites does not actually contain the stated fact.\n\n"
                ...
                "  3. Regulatory Triggers — a claim is SUPPORTED if Section 12 contains ANY "
                "mention of the same regulatory body, investigation, fine, framework, or deadline, "
                "even if the exact wording differs. For example: 'under ICO investigation' is "
                "supported if Section 12 mentions the ICO, a data-protection probe, or any related "
                "UK data-privacy enforcement action. Only flag as unsupported if the regulatory "
                "body or event is completely absent from Section 12.\n"
                "  4. AI Urgency\n"

...
def compute_domain_groundedness(session_state: dict[str, Any]) -> float:
    """Score evidence grounding for each populated domain output."""
    from src.worker.agents.tools.verification import Bm25Verifier
    from src.worker.domain.contracts import DOMAIN_OUTPUT_KEYS

    verifier = Bm25Verifier()
    populated = 0
    passed = 0
    ...
    for key in DOMAIN_OUTPUT_KEYS:
        ...
        if not val or not str(val).strip():
            continue
        populated += 1
        res = verifier.verify(str(val), session_state, agent_name="ParallelSearchAgent")
        if res.status == "PASSED":
            passed += 1

    if populated == 0:
        return 1.0
    score = passed / populated
    logger.debug(
        f"[Evaluation] M6 domain groundedness: {passed}/{populated} domains passed -> {score:.3f}"
    )
    return score

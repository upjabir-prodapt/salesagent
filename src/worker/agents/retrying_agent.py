...
from src.worker.domain.contracts import get_output_key, is_tracked_agent
from src.worker.runtime.resilience import (
...
from src.worker.runtime.state_mutation import is_mutable_state

from .tools.output_persistence import (
    has_nonempty_output,
...
class RetryingLlmAgent(LlmAgent):
        ...
        while True:
            try:
                ...
                if reason is not None:
                    logger.warning(
                        f"[Retry] Leaf error for agent={self.name} not retried ({reason}): {exc}"
                    )
                    raise
                ...
                if state is None or max_retries_exceeded(state, self.name):
                    logger.error(
                        f"[Retry] Leaf retry exhausted for agent={self.name} after {settings.AGENT_RETRY_ATTEMPTS} attempts"
                    )
            ...
            if max_retries_exceeded(state, self.name):
                logger.error(
                    f"[Retry] Leaf retry exhausted for agent={self.name} without output '{output_key}'"
                )
            ...
            await self._schedule_leaf_retry(
                ctx, state, Exception(f"Missing required output '{output_key}'")
            )

    async def _schedule_leaf_retry(
        self, ctx: InvocationContext, state: dict[str, Any], exc: Exception
    ) -> None:
        attempt = increment_retry_count(state, self.name)
        output_key = self._output_key()
        hint = self._build_retry_hint(output_key, exc)
        set_retry_hint(state, self.name, hint)
        prepare_agent_retry(state, self.name)
        wait_seconds = float(settings.AGENT_RETRY_WAIT_FIXED)
        logger.warning(
            f"[Retry] Retrying leaf agent={self.name} attempt={attempt}/{settings.AGENT_RETRY_ATTEMPTS} output_key={output_key!r} error={exc}"
        )
        if wait_seconds:
    ...
    def _persist_output_if_needed(
        self, ctx: InvocationContext, state: dict[str, Any], output_key: str
    ) -> None:
    ...
    def _clear_retry_artifacts(self, ctx: InvocationContext) -> None:
        ...
        if state is None:
            return
        pop_retry_hint(state, self.name)
    ...
    def _as_missing_output_error(self) -> AgentOutputError:
        ...
        return AgentOutputError(
            f"Leaf agent '{self.name}' exhausted retries without populating required output '{output_key}'.",
            agent_name=self.name,
            output_key=output_key,
    ...
    def _as_agent_output_error(self, exc: Exception) -> AgentOutputError:
        ...
        return AgentOutputError(
            f"Leaf agent '{self.name}' failed before producing required output '{output_key}': {exc}",
            agent_name=self.name,
            output_key=output_key,
    ...
    def _build_retry_hint(self, output_key: str, exc: Exception) -> str:
        return f"Previous attempt for {self.name} failed with error: {exc}. Re-run from the beginning and ensure final output populates '{output_key}'."

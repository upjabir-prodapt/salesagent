...
def create_plan_react_agent(
    ...
    *,
    ...
    """PlanReAct LlmAgent with verify_draft_answer and optional web search / extra tools.

    Prompts are self-contained (role → PlanReAct → output). Pass instruction_builder
    only to transform the prompt (e.g. legacy wrapping); default uses instruction as-is.
    """
    if output_key is None:
        output_key = f"{name.lower()}_output"

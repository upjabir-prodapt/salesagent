...
from src.agents.salesAgent.prompts import FIRMOGRAPHICS_PROMPT, EXECUTIVE_PROMPT

@pytest.fixture
def mock_agents():
    # We want to test the logic of the agents, not the LLM itself
    with patch("src.agents.salesAgent.sub_agents.research_agents.create_llm_agent") as mock_factory, \
         patch("src.agents.salesAgent.sub_agents.research_agents.SequentialAgent") as mock_sequential:
        ...
        def mock_seq_side_effect(name, sub_agents, description):
            ...
            m.run = MagicMock(side_effect=lambda *args, **kwargs: sub_agents[0].run(*args, **kwargs))
            return m

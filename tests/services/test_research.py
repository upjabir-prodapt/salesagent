...
async def test_process_research_background_success(research_service, mock_settings):
    ...
    with patch.object(
        ...
        with patch("src.agents.research_service.OutputGuardrail") as mock_guardrail_cls:
            ...
            with patch(
                "src.agents.research_service.EvaluationService"
            ) as mock_eval_cls:
                mock_eval_service = MagicMock()
                mock_eval_service.evaluate = AsyncMock(
                ...
                research_service.bigquery_repo.update_status.assert_any_call(
                    job_id,
                    "PROCESSING",
                    progress=mock_settings.RESEARCH_INIT_PROGRESS,
                    current_step=mock_settings.RESEARCH_INIT_STEP_LABEL,
                )
                research_service.gcs_repo.upload_json.assert_called_once_with(
                    job_id, session_state
                )
                research_service.gcs_repo.upload_markdown.assert_called_once_with(
                    job_id, final_report
                )
                research_service.bigquery_repo.update_status.assert_any_call(
                    job_id,
                    "COMPLETED",
                    gcs_uri=research_service.gcs_repo.upload_markdown.return_value,
                    progress=100,

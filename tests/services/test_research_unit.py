...
@pytest.mark.asyncio
async def test_process_research_background_full_flow(service, mock_repo, mock_gcs, mock_settings):
    ...
    with patch.object(service, "_run_sales_agent", new_callable=AsyncMock) as mock_run, \
        ...
        mock_eval.return_value.evaluate = AsyncMock(return_value={"final_composite_score": 0.8})

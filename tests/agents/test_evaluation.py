...
async def test_evaluate_success(evaluation_service):
    ...
    with (
        patch.object(
            evaluation_service, "_run_section_a", new_callable=AsyncMock
        ) as m1,
        patch.object(
            evaluation_service, "_run_section_b", new_callable=AsyncMock
        ) as m2,
    ):
...
async def test_evaluate_section_a_failure(evaluation_service):
    ...
    with (
        patch.object(
            evaluation_service, "_call_llm_judge", new_callable=AsyncMock
        ) as m1,
        patch.object(
            evaluation_service, "_run_section_b", new_callable=AsyncMock
        ) as m2,
        ...
        result = await evaluation_service.evaluate(
            request_id="job_123", final_report="# Test Report", session_state={}
        )

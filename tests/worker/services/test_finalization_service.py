...
async def test_finalize_runs_all_ops_successfully() -> None:
    ...
    with (
        patch(
            "src.worker.services.finalization_service.run_pdf_op",
            new_callable=AsyncMock,
            return_value=True,
        ),
        patch(
            "src.worker.services.finalization_service.run_evaluation_op",
            new_callable=AsyncMock,
        ),
        patch(
            "src.worker.services.finalization_service.run_cost_attribution_op",
            new_callable=AsyncMock,
        ),
        patch(
            "src.worker.services.finalization_service.run_telemetry_flush_op",
            new_callable=AsyncMock,
        ),
    ):

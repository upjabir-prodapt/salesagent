...
from src.worker.runtime.pricing import SEARCH_AGENT_NAME
...
def test_gemini_35_flash_pricing_catalog() -> None:
    pricing_global = model_pricing.get_model_pricing("gemini-3.5-flash")
    assert pricing_global is not None
    assert pricing_global.input_per_1m == pytest.approx(1.50)
    assert pricing_global.output_per_1m == pytest.approx(9.00)
    assert pricing_global.context_window_tokens == 1048576
    assert pricing_global.search_cost_per_1k == 35.0

    pricing_regional = model_pricing.get_model_pricing(
        "gemini-3.5-flash", region="europe-west3"
    )
    assert pricing_regional is not None
    assert pricing_regional.input_per_1m == pytest.approx(1.65)
    assert pricing_regional.output_per_1m == pytest.approx(9.90)
    assert pricing_regional.region == "europe-west3"

    cost_regional = model_pricing.calculate_token_cost(
        "gemini-3.5-flash", 1_000_000, 1_000_000, region="europe-west3"
    )
    assert cost_regional == pytest.approx(11.55)
